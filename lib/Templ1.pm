...
return Templ1::MenuClient->menu($c, $ref_hash_dir_subs);
}#--------------
    ...
    my $result = $db->select( $table, ['url', 'id'] );
        ...
        while( my($pattern, $id) = $result->list ){
            ...
            if( $table eq 'level0' || $table eq 'level1' ){
                ...
                $db->update( $table, {menu => "$curr_menu"}, {id => "$id"} );
            }else{
                $db->update( $table, {menu => "$menu"}, {id => "$id"} );
            }
...
#************************************
    ...
    $message_of_comment = MyBlog::CommentForm->comment_form( $c, $table, $title_alias, 'templ1/index' );
    if( $c->session('client') ){
        MyBlog::AnswerForm->answer_form( $c, $table, $title_alias, 'templ1/index' );
    }
    $comments_tree = MyBlog::CommentsTree->comments_tree($c, $page_id);

}#**************

$message = MyBlog::AuthorizeForm->authorize_form( $c, 'templ1/index' );
...
my $ref_id_name_rubric = Templ1::RubricList->rubric_list($c);
...
$content->[0]->{'rubric'} = uc $rubric if $rubric;
    ...
    if($content->[0]->{'comment_enable'} eq 'yes'){
        ...
        $message_of_comment = MyBlog::CommentForm->comment_form( $c, $table, $title_alias, $id, 'templ1/index' );
        if( $c->session('client') ){
            MyBlog::AnswerForm->answer_form( $c, $table, $title_alias, 'templ1/index' );
        }
